...
<head>
  ...
  <style>
    ...
    .pdf-button { align-items:center; background:#fff !important; border:1px solid #d1d5db; border-radius:6px; box-shadow:none; color:#111827 !important; display:inline-flex; font-weight:800; gap:7px; padding:9px 14px; text-decoration:none; }
    .pdf-button:hover { background:#f9fafb !important; border-color:#9ca3af; color:#111827 !important; text-decoration:none; }
...
<body>
  ...
  <div class="page-content"><div class="page-header"><div class="container-fluid"><main class="history">
    ...
    <form class="history-filter" method="GET" action="{{ route('admin.transaction-history') }}"><label>From <input type="date" name="from" value="{{ $from->toDateString() }}"></label><label>To <input type="date" name="to" value="{{ $to->toDateString() }}"></label><button type="submit">Apply range</button><a class="pdf-button" href="{{ route('admin.transaction-history.pdf', request()->only(['from', 'to'])) }}"><i class="fa fa-file-pdf-o"></i> Download PDF</a></form>
